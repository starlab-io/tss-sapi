...
mod sys {
    ...
    impl From<TPMS_AUTH_COMMAND> for TSS2_SYS_CMD_AUTHS {
        fn from(cmd: TPMS_AUTH_COMMAND) -> Self {
            TSS2_SYS_CMD_AUTHS::new(vec![cmd]).unwrap()
        }
    }
...
impl NvRamArea {
    ...
    pub fn define(ctx: &Context,
        ...
        let mut session_out = sys::TSS2_SYS_RSP_AUTHS::from(resp);

        trace!("Tss2_Sys_NV_DefineSpace({:?}, AUTH, SESSION_DATA, NV_AUTH, {:?}, SESSION_OUT)",
               ctx.inner,
               nv);
        tss_err(unsafe {
